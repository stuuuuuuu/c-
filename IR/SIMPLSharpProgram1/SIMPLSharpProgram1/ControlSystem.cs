    ...
    public class ControlSystem : CrestronControlSystem
    {
        ...
        void userinterface_SigChange(BasicTriList currentDevice, SigEventArgs args)
        {
            ...
            switch (args.Sig.Type)
            {
                case eSigType.Bool:
                    {
                        this.ProcessTsw760Bool(args.Sig.Number, args.Sig.BoolValue);
                    }
                    break;
        ...
        private void ProcessTsw760Bool(uint sigNumber, bool boolValue)
        {
            if (boolValue)
            { 
               TSW760Bool t = (TSW760Bool)sigNumber;
               UDPAPI udp = new UDPAPI();
               string light_conctrol;
               switch (t)
               {
                   case TSW760Bool.light_open:
                       userinterface.BooleanInput[(uint)TSW760Bool.checek].BoolValue = true;
                       light_conctrol = "ssss";
                       udp.SendData("192.168.188.112",8080, light_conctrol);
                       break;
                   case TSW760Bool.shut_down:
                       userinterface.BooleanInput[(uint)TSW760Bool.checek].BoolValue = false;
                       light_conctrol = "ssss";
                       udp.SendData("192.168.188.112",8080, light_conctrol);
                       break;
                   default:
